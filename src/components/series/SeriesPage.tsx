...
async function fetchSeriesPage(tab: string, page: number): Promise<TVResult[]> {
  switch (tab) {
    ...
    case "Trending":
      return fetchTrendingTV("day");
    case "On Air":
      return fetchOnAirTV(page);
    default:
...
const SeriesPage = () => {
  ...
  return (
    ...
    >
      ...
      <main className="w-full mt-8 sm:mt-12 mb-8 sm:mb-10">
        <div className="max-w-[90rem] mx-auto px-3 sm:px-5 lg:px-8">
            ...
            <div className="flex gap-2 flex-wrap">
              {SERIES_TYPES.map((type) => (
                <button
                  ...
                  disabled={isLoading}
                  className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-semibold font-roboto transition-all duration-200 disabled:opacity-60 ${
                    select === type.label
                      ? "text-white"
                      : "bg-white/5 text-white/80 hover:bg-white/10 hover:text-white border border-white/10"
                  }`}
                  style={
                    select === type.label ? { background: "var(--accent)" } : {}
                  }
                >
                  {type.label}
                </button>
          ...
          {!hasNextPage && !isLoading && allItems.length > 0 && (
            <p className="text-center text-white/55 text-sm font-roboto py-8">
