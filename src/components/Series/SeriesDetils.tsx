    ...
    return (
        <div className="flex flex-wrap gap-2">
            {genres.map((g) => (
                <button
                    key={g.id}
                    onClick={() => navigate(`/genres/${g.id}`, { state: { searchValue: g.name } })}
    ...
    return (
        <section className="mt-10 sm:mt-14">
            <h2 className="text-xl font-bold font-roboto text-white mb-5">
                Seasons
                <span className="text-white/30 font-normal text-base ml-2">({visible.length})</span>
            </h2>
                ...
                {visible.map((s) => (
                    ...
                    >
                        ...
                        <div className="relative w-full aspect-[2/3] overflow-hidden flex-shrink-0 bg-white/5">
                            {s.poster_path ? (
                                ...
                                <div className="w-full h-full flex items-center justify-center">
                                    <span className="text-white/20 text-4xl font-bold font-roboto">
                                        {s.season_number}
                                    </span>
                                </div>
                            )}
                        ...
                        <div className="px-2 pt-2 pb-3">
                            <p className="text-white text-xs font-semibold font-roboto line-clamp-1">{s.name}</p>
                            {s.air_date && (
...
function CastRow({ cast }: { cast: any[] }) {
    ...
    return (
        <section className="mt-12">
            <h2 className="text-xl font-bold font-roboto text-white mb-5">Top Cast</h2>
            <div
                className="flex gap-3 sm:gap-4 overflow-x-auto no-scrollbar pb-2"
                style={{ touchAction: "pan-x" }}
            >
                {top.map((person: any) => (
                    <div key={person.id} className="flex-shrink-0 w-24 sm:w-28 text-center">
                        ...
                        {person.roles?.[0]?.character && (
                            <p className="text-white/35 text-[10px] font-roboto line-clamp-1 mt-0.5">
                                {person.roles[0].character}
                            </p>
                        )}
                    </div>
        ...
        <section className="mt-10 sm:mt-14">
            ...
            >
                {rows.map((row, i) => (
                    <div
                        key={row.label}
                        className={`flex gap-4 px-4 sm:px-6 py-3 text-sm font-roboto ${i < rows.length - 1 ? "border-b border-white/6" : ""
                            }`}
                    >
    ...
    if (!show) {
        return (
            ...
            >
                ...
                <p className="text-white/40 font-roboto text-lg">Series not found.</p>
            </div>
        );
    }
            ...
            <div className="max-w-6xl mx-auto px-3 sm:px-6 lg:px-10">
                ...
                <div className="flex flex-col sm:flex-row gap-5 lg:gap-10 -mt-24 sm:-mt-36 lg:-mt-48 relative z-10">
                    ...
                    <div className="flex-1 flex flex-col gap-4 pt-2 sm:pt-10 lg:pt-14 min-w-0">
                        ...
                        <button
                            onClick={() => navigate(-1)}
                            className="self-start flex items-center gap-1.5 text-white/40 hover:text-white transition-colors text-xs font-roboto mb-1"
                        >
                        ...
                        <div>
                            ...
                            {show.tagline && (
                                <p className="text-white/30 font-roboto text-sm italic mt-1">
                        ...
                        {/* Rating */}
                        <RatingRing vote_average={show.vote_average} vote_count={show.vote_count} />
                ...
                {show.overview && (
                    <div className="sm:hidden mt-6">
                        ...
                        <p className="text-white/75 font-roboto text-sm leading-relaxed">{show.overview}</p>
                    </div>
                )}
