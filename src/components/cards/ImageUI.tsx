...
export type MediaItem = (MovieResult | TVResult) & {
  _mediaType?: "movie" | "tv";
};
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2.5 sm:gap-3 lg:gap-4 w-full mb-3">
      {data.slice(0, 10).map((p) => (
        <MovieCardItem
          key={p.id}
          movie={p}
          mediaType={p._mediaType ?? mediaType}
        />
      ))}
    </div>
  );
...
function getDisplayTitle(item: MediaItem): string {
  ...
  if ("original_name" in item && (item as TVResult).original_name)
    return (item as TVResult).original_name;
...
const MovieCardItem = memo(function MovieCardItem({
  ...
  return (
    ...
    >
      ...
      >
        <img
          src={getImagePath(185, p.poster_path)}
          ...
          style={{
            backgroundImage: `url(${getTinyPosterPath(p.poster_path)})`,
