import { PlayIcon, StarIcon } from "@/lib/icons/icons";
import {
  getBackdropSrcSet,
  getImagePath,
  getLargeImagePath,
...
const HeroSection = ({ movies, loading }: HeroSectionProps) => {
  ...
  return (
    <div
      className="relative w-full h-[55vw] min-h-[260px] max-h-[660px] overflow-hidden bg-[#0e0e12] select-none"
      role="region"
    ...
    >
      ...
      <div
        aria-live="polite"
        aria-atomic="true"
        className={`absolute bottom-10 sm:bottom-14 md:bottom-18 left-4 sm:left-10 lg:left-16 max-w-[min(88%,560px)] transition-all duration-300 ${
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-2 sm:mb-3">
          <span
            className="px-2 sm:px-2.5 py-0.5 text-[10px] sm:text-xs font-bold font-roboto rounded-full text-white tracking-wide uppercase"
        ...
        <div className="flex items-center gap-2 sm:gap-3 mt-2 sm:mt-0">
          ...
          >
            <span aria-hidden="true" className="w-3.5 h-3.5 sm:w-4 sm:h-4">
              <PlayIcon />
            </span>
            Watch Now
          </button>
      ...
      <div className="hidden lg:flex absolute right-6 xl:right-12 top-1/2 -translate-y-1/2 flex-col gap-2.5">
        {featured.map((m, i) => (
          <button
            ...
            style={
              i === current ? { boxShadow: "0 0 0 2px var(--accent)" } : {}
            }
          >
            <img
              src={getImagePath(92, m.poster_path)}
              srcSet={getPosterSrcSet(m.poster_path)}
              sizes="64px"
              ...
              fetchpriority={i === 0 ? "high" : "low"}
              style={{
                backgroundImage: `url(${getTinyPosterPath(m.poster_path)})`,
                backgroundSize: "cover",
              }}
